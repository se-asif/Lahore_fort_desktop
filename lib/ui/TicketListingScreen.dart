...
class _TicketListingScreenState extends State<TicketListingScreen> {
  ...
  Future<List<VisitingDataList>> _fetchVisitData(
      {DateTime? from, DateTime? to}) async {
    List<VisitingDataList> allData =
        await DatabaseHelper.instance.getVisitingData();
      ...
      return allData.where((ticket) {
        try {
          ...
          return (ticketOnlyDate.isAtSameMomentAs(fromOnlyDate) ||
              ...
              (ticketOnlyDate.isAfter(fromOnlyDate) &&
                  ticketOnlyDate.isBefore(toOnlyDate)));
        } catch (e) {
  ...
  Future<List<VisitingDataList>> _fetchPendingVisitData(
      {DateTime? from, DateTime? to}) async {
    List<VisitingDataList> allData =
        await DatabaseHelper.instance.getVisitingData();
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          ...
                                          Consumer<TicketsProvider>(
                                            builder:
                                                (context, provider, child) {
                                              if (ticket.isUploaded
                                                      .toString() ==
                                                  "0") {
                                                return ElevatedButton(
                                                  onPressed: () async {
                                                    if (await HelperFunction
                                                        .hasInternetConnection()) {
                                                      provider
                                                          .postVisitingDataFromDB(
                                                              ticket);
